...
function MexUpgradeBehaviour:OwnerIdle()
	if self:IsActive() then
		...
		if not self.released then
			self.assisthandler:Release(builder)
			self.released = true
		end
		...
		if self.mexPos ~= nil and not self.mohoStarted then
			...
			if s then
				...
				self.assisthandler:PersistantSummon(builder, self.mexPos, helpList[mohoName])
				self.released = false
				self.active = true
	...
	for _, unit in pairs(ownUnits) do
		...
		if mexUpgrade[un] then
			...
			if selfUnit:CanBuild(upgradetype) then
				...
				if self.maphandler:UnitCanGetToUnit(selfUnit, unit) then
					local distMod = 0
					...
					if self.targethandler:IsSafePosition(pos, selfUnit) then
