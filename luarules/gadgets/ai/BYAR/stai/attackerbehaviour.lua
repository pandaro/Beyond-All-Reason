...
function AttackerBehaviour:OwnerBuilt()
	...
	self.attackhandler:AddRecruit(self)
end
...
function AttackerBehaviour:OwnerDead()
	...
	self.active = nil
	self.unit = nil
	self.attackhandler:NeedMore(self)
	self.attackhandler:RemoveRecruit(self)
	self.attackhandler:RemoveMember(self)
end
...
function AttackerBehaviour:OwnerIdle()
	...
	self.timeout = nil
	if self.active then
		self.attackhandler:MemberIdle(self)
	end
end
...
function AttackerBehaviour:Advance(pos, perpendicularAttackAngle, reverseAttackAngle)
	...
	if reverseAttackAngle then
		...
		if not self.sturdy or self.loshandler:IsInLos(pos) then
	...
	local canMoveThere = self.maphandler:UnitCanGoHere(self.unit:Internal(), self.target)
	...
	elseif self.squad.lastValidMove then
		...
		canMoveThere = self.maphandler:UnitCanGoHere(self.unit:Internal(), self.target)
	end
